...
const CONFIG = {
  // IMPORTANT: PayPal תשלום "למייל" לא עובד כלינק checkout.
  // צריך PayPal.me USERNAME (למשל PerfectGamingStore) או לינק כפתור/אינוויס.
...
function initPay(){
  ...
  const afterPayBtn = document.getElementById("afterPayBtn");

  if (afterPayBtn) afterPayBtn.href = `success.html?order=${encodeURIComponent(order.orderId)}`;

  const paypalLink = buildPayPalLink(order.amount);
  const bitLink = buildBitLink(order.amount, order.orderId);

  if (paypalBtn){
    if (paypalLink){
      paypalBtn.href = paypalLink; paypalBtn.target="_blank"; paypalBtn.rel="noopener";
    } else {
      paypalBtn.href="#";
      paypalBtn.addEventListener("click",(e)=>{e.preventDefault(); alert("שים PAYPAL_ME_USERNAME ב‑app.js כדי לחבר PayPal.me.");});
    }
  }
  if (bitBtn){
    // Bit locked for now (coming soon)
    // When you are ready: replace the button in pay.html back to an <a> and set BIT_PAYMENT_URL in app.js.
  }
}
